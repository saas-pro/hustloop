  ...
  <div className="w-full overflow-visible pointer-events-none flex justify-center items-end h-[120px] translate-y-4">
    <div className="relative w-full max-w-5xl flex justify-center scale-90 origin-bottom">
      ...
      <div className="group absolute translate-x-[300px] translate-y-12 hover:z-50 pointer-events-auto cursor-pointer">
        {/* Trigger Card */}
        <div className="w-[300px] h-[160px] rounded-[24px] bg-background text-foreground shadow-2xl rotate-[10deg] flex flex-col justify-center px-8 border border-border transition-all duration-300">
          <div className="inline-block py-0.5 rounded-full text-amber-600 font-bold uppercase tracking-widest mb-2">Best Value</div>
...
export default function WeFillPage() {
  ...
  const submitForm = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/wefill/apply`, {
        method: 'POST',
        body: formData
      });
    ...
    if (q.type === 'welcome') {
      return (
        <div className="flex flex-col items-center text-center my-auto py-12">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="text-xs text-muted-foreground/60 mt-6 text-center">
            By continuing, you agree to our <a href="/terms-of-service" target="_blank" className="underline hover:text-foreground transition-colors">Terms and Conditions</a> and <a href="/privacy-policy" target="_blank" className="underline hover:text-foreground transition-colors">Privacy Policy</a>.
          </motion.div>
        </div>
      );
    }

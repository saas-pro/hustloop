...
export default function EducationView({ isOpen, onOpenChange, onApplyClick }: EducationViewProps) {
  return (
                    ...
                    <div className="space-y-8">
                        {acceleratorPrograms.map((program, index) => (
                            <Card key={index} className="bg-card/50 backdrop-blur-sm border border-primary/30 hover:border-primary transition-colors">
                                ...
                                <CardFooter>
                                    <Button onClick={onApplyClick} className="w-full md:w-auto ml-auto bg-accent hover:bg-accent/90 text-accent-foreground">Apply Now</Button>
                                </CardFooter>
                            </Card>
                        ))}
                    </div>

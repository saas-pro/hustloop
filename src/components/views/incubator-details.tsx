...
export default function IncubatorDetails({ incubator, onOpenChange, isLoggedIn, hasSubscription }: IncubatorDetailsProps) {
  ...
  const handleApplyClick = () => {
    if (founderRole === "Submit an innovative idea") {
      if (!hasSubscription) {
  ...
  // If the user is an innovative idea founder without a subscription, the button should not be disabled
  // so they can click it and be navigated to the pricing page.
  // const isDisabled = !isLoggedIn || (!isInnovativeIdeaFounder && !hasSubscription);
  // let tooltipContent = null;
  // if (!isLoggedIn) {
  //   tooltipContent = <p>Please login to apply for incubation</p>;
  // } else if (!isInnovativeIdeaFounder && !hasSubscription) {
    ...
    <>
      ...
      <Dialog open={isApplyDialogOpen} onOpenChange={setIsApplyDialogOpen}>
        <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
          <FounderIdeaSubmission incubatorId={incubator.id} />
        </DialogContent>
      </Dialog>
    </>
  );
